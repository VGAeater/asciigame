...
def approximate_color( c ):
    ...
    for back in colors[:8]:
        for fore in colors:
            ...
            # distance, background color, foreground color, percent between them, final color
            entry = ( approx_dist, back, fore, ddist / dist if dist != 0 else 0, di )
...
for i in range( im.size[0] ):
    for j in range( im.size[1] ):
        color = approximate_color( data[i, j] )[4].tolist();
        color = tuple( [ int( v ) for v in color ] )
        data[i, j] = color

im.show()

print( binarySearch( brightness, 0.85 ) )
